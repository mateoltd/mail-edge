...
import type {
  ResendFeedbackWireEvent,
  ResendProviderConfig,
  ResendWebhookReplayStore,
} from "./types.js";
...
export class ResendFeedbackAdapter implements FeedbackProviderAdapter {
  ...
  readonly #replay: ResendWebhookReplayStore;
  ...
  async ingestFeedback(
    ...
    if (replay.value === "conflict") {
      return { error: resendError("CONFLICT", "feedback_event_conflict"), ok: false };
    }
    if (replay.value === "duplicate") return { ok: true, value: Object.freeze([]) };
