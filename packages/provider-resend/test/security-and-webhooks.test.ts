...
describe("Resend security and webhooks", () => {
  ...
  it("normalizes recipient feedback and returns replay identity for atomic durable dedupe", async () => {
    const registration = await createStartedRegistration();
    const body = json({
      created_at: NOW,
      data: {
    ...
    expect(first.ok && first.value.events).toHaveLength(2);
    if (first.ok) {
      expect(first.value.events.map((event) => event.recipient)).toEqual([
        "one@example.test",
        "two@example.test",
      ]);
      expect(first.value.events.every((event) => event.kind === "complained")).toBe(true);
      expect(first.value.replay?.providerInstanceId).toBe(ingressContext().providerInstanceId);
    }
    const duplicate = await ingest(requestFor(body, { eventId: "feedback-replay" }));
    expect(duplicate.ok && duplicate.value.events).toHaveLength(2);
    if (first.ok && duplicate.ok) expect(duplicate.value.replay).toEqual(first.value.replay);
    ...
    if (first.ok && conflict.ok) {
      expect(conflict.value.replay?.nonceDigest).toBe(first.value.replay?.nonceDigest);
      expect(conflict.value.replay?.bodyDigest).not.toBe(first.value.replay?.bodyDigest);
    }
  });
  ...
  it.each([
    ...
    if (result.ok) {
      ...
      expect(result.value.events[0]?.kind).toBe(kind);
      expect(result.value.events[0]?.normalizedEvidence).toMatchObject({
        authenticated: true,
