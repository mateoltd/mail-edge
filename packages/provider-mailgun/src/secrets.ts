...
/** Resolves one UTF-8 secret and clears the resolver-owned byte copy. @internal */
export const resolveSecretText = async (
  ...
  try {
    ...
    if (text.length < 1 || text.length > 4096 || /[\r\n\0]/u.test(text)) {
      return { error: mailgunError("AUTHENTICATION_FAILED", "secret_shape"), ok: false };
    }
    return { ok: true, value: text };
  } catch (cause) {
    return {
      error: mailgunError("AUTHENTICATION_FAILED", "secret_encoding", false, cause),
      ok: false,
    };
  } finally {
